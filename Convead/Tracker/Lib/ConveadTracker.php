...
class ConveadTracker {
  ...
  private function put_log($url, $method, $post, $headers, $response = true) {
      ...
      $this->addRecord("REQUEST: {$method} {$url}");
      $this->addRecord("HEADER DATA:", $headers);
      $this->addRecord("POST DATA:", $post);
      $this->addRecord("RESPONSE:", $response);
      $this->addRecord("====================");
     ...
     * @return void
     */
    private function addRecord($message, $data = [])
    {
        if (is_string($data)) {
            $data = [$data];
        ...
        \Magento\Framework\App\ObjectManager::getInstance()
            ->get(\Convead\Tracker\Logger\Logger::class)
            ->debug($message, $data);
    }
}
